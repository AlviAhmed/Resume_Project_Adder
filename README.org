...
- Need to make sure projects in buffer are in proper format so that
  their fields can be parsed by awk function properly
- if the buffer file is empty at a line to the top, need this to
  ensure all other projects have the correct fields in the awk function
- With repeat function, no matter how times 1 projects repeats, awk
  script will input only 1 instance on them, however to manage
